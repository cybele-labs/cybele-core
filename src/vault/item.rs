    ...
    pub(crate) fn serialize_into(&self, w: &mut Vec<u8>) {
        match self.version {
            Version::Test | Version::V1 => {
                ...
                w.write_all(&[self.encrypted_value.len() as u8]).unwrap();
    ...
    pub(crate) fn deserialize_from(version: Version, r: &mut BufReader<&[u8]>) -> Option<VaultItem> {
        match version {
            Version::Test | Version::V1 => {
                ...
                // [u8: encrypted_value_len]
                let mut encrypted_value_len_bytes = [0u8];
                r.read_exact(&mut encrypted_value_len_bytes).ok()?;
                let encrypted_value_len = encrypted_value_len_bytes[0] as usize;
                // [encrypted_value_len*u8: encrypted_value]
...
mod tests {
    ...
    fn serialize_vault_item() {
        ...
        assert_eq!(
            ...
            "0009346368616e20707764000102030405060708090001020304050607080900010203040506070809000104deadbeef"
        );
        assert_eq!(item, deserialized.unwrap());
    }

    #[test]
    fn serialize_vault_item_long_name() {
        let item = VaultItem {
            version: Version::V1,
            ...
            salt: [42u8; 32],
            encrypted_value: hex::decode("03958e0a08d2d23e708d0b0778c87c83140e089fdf9089").unwrap(),
        };
        let serialized: Vec<u8> = item.serialize();
        let deserialized = VaultItem::deserialize(Version::V1, &serialized);
        assert_eq!(hex::encode(item.serialize()), "");
        assert_eq!(item, deserialized.unwrap());
    }
